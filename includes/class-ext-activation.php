...
class Users_WP_Extension_Activation {
    ...
    public function __construct( $plugin_path, $plugin_file ) {
        ...
        if ( isset( $plugins[$this->plugin_path . '/' . $this->plugin_file]['Name'] ) ) {
            ...
            $this->plugin_name = __( 'This plugin', 'uwp-recaptcha' );
        }
    ...
    public function missing_uwp_notice() {
        if ( $this->is_uwp_installed ) {
            echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires %sUsersWP%s. Please activate it to continue.', 'uwp-recaptcha' ), '<a href="https://wpgeodirectory.com/users-wp/" title="UsersWP" target="_blank">', '</a>' ) . '</p></div>';
        } else {
            echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires %sUsersWP%s. Please install it to continue.', 'uwp-recaptcha' ), '<a href="https://wpgeodirectory.com/users-wp/" title="UsersWP" target="_blank">', '</a>' ) . '</p></div>';
        }
    }
}